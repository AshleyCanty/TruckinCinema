...
class TabBarController: UITabBarController, UITabBarControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let tabBarTopBorder = UIView()
        tabBarTopBorder.backgroundColor = AppColors.BackgroundSecondary
        
        tabBar.backgroundColor = AppColors.BackgroundMain
        tabBar.addSubview(tabBarTopBorder)
        tabBarTopBorder.disableTranslatesAutoresizingMaskIntoContraints()
        tabBarTopBorder.heightAnchor.tc_constrain(equalToConstant: 1)
        tabBarTopBorder.topAnchor.tc_constrain(equalTo: tabBar.topAnchor)
        tabBarTopBorder.leadingAnchor.tc_constrain(equalTo: tabBar.leadingAnchor)
        tabBarTopBorder.trailingAnchor.tc_constrain(equalTo: tabBar.trailingAnchor)
    }
    ...
    fileprivate func addTabBarItemStyle(tabController: UINavigationController) {
        ...
        tabController.tabBarItem.setTitleTextAttributes([.font: AppFont.semiBold(size: 10)], for: .normal)
        tabController.tabBarItem.setTitleTextAttributes([.font: AppFont.semiBold(size: 10)], for: .selected)
    }
    
    fileprivate func setStyle() {
        tabBar.barTintColor = AppColors.BackgroundMain
        ...
        if #available (iOS 15, *) {
            ...
            appearance.backgroundColor = AppColors.BackgroundMain
            tabBar.standardAppearance = appearance
            tabBar.scrollEdgeAppearance = tabBarController?.tabBar.standardAppearance
        }
    }
}
